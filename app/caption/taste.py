...
2) authored: complete standalone captions the operator WROTE THEMSELVES inside the note (a full line they'd post, usually followed by "would have been an 8/9/10") that match NO listed candidate. The test: posted alone with nothing around it, would this text be a complete post? A noun phrase, a replacement ending for part of another caption ("has to be something like ..."), or a premise template with blanks FAILS the test — return those in NEITHER list. For each pass: the line verbatim exactly as written in the note and the claimed rating (null if none). Empty list if none.
