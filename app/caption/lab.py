...
- PLAY: how it's DELIVERED — a moment the reader lands INSIDE mid-beat (the way a "POV:", a "when…", a snapped line of dialogue does), an exchange where the punch is said to someone, a build the reader walks into, an energy spike, a structure nobody's used before. Dropped-in, never NARRATED: the second a scene stages itself like a short story ("he looked me dead in the eyes", "everyone stood up") it's fan-fiction, not a post. INVENT shapes; the codex's form principles tell you what makes a shape his. The play and the premise should need each other.
...
THE TASK: below are {k} locked IDEAS — each a PREMISE plus its PLAY, ideated from your codex. The premises are FIXED (never swap, merge, or drift them); the play tells you the delivery — commit to it or invent a sharper shape for the same premise. Your catalog above is the BAR and the sound-check, not source material: its premises are taken, but it shows exactly how you actually type — the casing, the slang, the emoji, the energy, the timing. Write each idea AS A POST, in that exact texture. If a draft reads like a clean observation an essayist could have written, it's DEAD — that's not you; rewrite it as the bit it wants to be. Two more dead-on-arrival reads: a decode that lands MATTER-OF-FACT ("X is just Y", stated cleanly, walking away — the landing has to snap, not explain), and a scene that reads NARRATED like fan-fiction (staged beats, dramatic eye contact, a room reacting on cue — you drop the reader into a moment, you never perform one). Write the strongest {n} of the {k}; drop ideas you can't make catalog-topping. Only ship what you'd bet on.
