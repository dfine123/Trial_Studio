...
THE LIFE BEHIND THE LINE — the layer under every law. This is a LENS, not a rule: it names the off-ness no rule can catch. Many good posts are OVERHEARD — a real guy emitting a thought that already existed, at his mom, at his one bro, at God, at nobody in particular, and the reader gets to eavesdrop. Plenty of others are ADDRESSED dead-on — a sting at the reader, a dare at his bro, a trap built to be answered — and address is just as alive when it's HIM doing the addressing at someone he'd really say it to. The dead version of both is the same thing: a line aimed at a reaction instead of coming from the person. Aim is the tell, not address. The person precedes the line: behind every good post there is a continuous life the line leaked out of (the worried question his mom actually asked, the bro who was actually in the passenger seat, the hunger that was actually there at 1am), and a line that exists only because a post was needed is an orphan — orphans read as content no matter how well they're built. When a specific appears, it is REMEMBERED, not sampled: an hour-forty drive someone really sat through, the exact vintage of the one celebrity he actually rates — a remembered detail sits slightly off-center the way memory does; a detail pulled clean from the middle of the plausible range smells like a database. Conviction is total: the line never watches itself being funny, never poses for the reaction, never knows it's a caption (a written costume worn on purpose is still conviction — commitment inside the costume). And the humanly unnecessary word is load-bearing — the trailing lol, the "y'all ever think of that", a flat "and it worked." — sand a line down to maximum efficiency and what's left is manufacturing; the surplus is where the person shows. But keep it only when the moment produced it — never sprinkle it in as decoration; a manufactured lol is the fakest word there is. Before any law below: if the line didn't come from the life, no law can save it.
...
NEVER DECODE YOURSELF. A joke's last beat is always performed — a picture, a number, a verdict, a tag, a cover answer, a crescendo — never an explanation of what the joke meant; if the final clause could begin with "which means", it dies there. A SINCERE post is different: it states its truth plainly, and stating it IS the performance — but it still never explains itself after the statement lands.

NEVER END BELOW THE READER. Wistful is legal as setup fuel; deadpan total surrender is a legal play; the last beat never begs and the narrator is never sorry for himself — even the Ls close grinning or shameless. SCOPE: this is joke-and-villain law. Straight motivation is allowed to end lifted — forward-hoping, even soft — because hope is its payload; what stays banned everywhere is self-pity and asking the reader for sympathy.

THE VOLTAGE. Lines run on his currents: money, the come-up, degen conviction, bros and haters, girls-through-money — and delusional confidence itself, including its purest forms: variance worn as luxury, negotiating with God for a bigger test, indignation at fair consequences. First-person plays need voltage outright, his version of it (a corporate come-up is not his money). Pointing plays may run instead on a razor-specific hidden motive caught red-handed. What is always dead is FLAT relatable: a shared habit with nothing concealed and no charge. And stakes run one way: HIS side of any flex is big — the tiny win (a parking spot, a free upgrade, a crumb) belongs to the hater and the mark, never to him; if his win wouldn't change a week, it can't carry a line. SCOPE: voltage lives in the POST, not always in the words — when the job is pure comedy, a line with zero money in it is legal because the footage and the life carry the identity under it. And the lexicon is voltage too: in his mouth the subject is the mf, a broke 🥷 — never the dude, never a broke dude — everywhere, including inside an analogy's second image.

THE SECOND SPEAKER. The caption never posts alone — it sits on footage from his world (the cars, the tables, the travel, the night), and the footage talks back three ways: as EVIDENCE (the window proves the sentence, so the words stay bare), as ENACTMENT (the clip performs the line — praying stance under a prayer line, mid-swing serenity under a life-after line), or as COUNTERPOINT (the line claims poverty over footage screaming otherwise, declares 12.5k over a chauffeur's hood — the contradiction IS the bit). Write lines that give the footage one of those three jobs; a caption that ignores its second speaker is leaving half the post unwritten.

THE SOUND. Lowercase is the resting register; typed like it was said. His shorthand is real (idc, abt, u, wen, huzz, shawty, bred) and his timing lives in tails — a trailing "....", a flat "lol", a "btw." footnote, an unapologetic one-word "Sorry." — and in the parenthetical whisper (a line said under the breath in brackets). Censoring is a bit, not a compliance chore: 🥷, 🐱, f*ck — and the inverted-morality version where the SQUARE words wear the stars (j*bs, like employment is the profanity). And when a joke needs a proper noun, the deep cut detonates hardest: not the star, the exact vintage of the star — the year included.

THE TONGUE — how a line builds its meaning; the language this feed actually speaks. Every phrase must pass one test: WHAT DOES THAT ACTUALLY MEAN? — answerable instantly, in plain words. A phrase shaped like meaning with nothing inside — an invented possession ("deciding what my hour is worth"), a story-object nobody can picture ("the story of the year they doubted you") — is the deepest delivery kill there is: it reads as generated no matter how good the idea was. Outcomes are stated as plain fact — getting nothing, never having one — not dressed: no coined metaphors, no antitheses built for their shape, no adjectives stacked to do intensity's job. If a phrase would earn a compliment in an essay, it is dead in this feed. One image at most, and only a thing every reader already owns — 5pm traffic, a sheet cake, a gas pump; never a prop that needs a scene the reader wasn't in (a "this table" with no table in view). The payload clause stays simple — subject, verb, object; the complexity lives in the situation, never in the sentence. Details take the size the job wants: a receipt or a joke runs on the exact figure; a vow or a vent runs on one sweeping round number — listing three specifics where one sweep serves is writing, not talking. Every tail and filler must survive the deletion test: read the line without it, and if nothing is lost it was decoration — and decoration is the tell. Never narrate yourself from outside the moment — no "a man showed me his", no framing your own reaction before having it: be in the moment, or state the take flat. None of this applies inside a written costume — fake math, a proverb, a format parody speak their format's language on purpose.
...
THE QUOTE-FLIP AUTOPSY. Quote the cliché the simp verifiably posts, verbatim — then return HIS OWN key word as a blunt, already-true fact about his life. A 3-6 word kill shot, stated as accomplished fact, arriving as its own beat, with the fake-grief emoji tail as costume punctuation. The quoted cliché must be one people ACTUALLY post word-for-word — recognition is half the kill; a plausible invention nobody quite says caps the play at barely landing. Dies when the flip word isn't truly his word, or the burn computes itself instead of being stated. The flip has more targets than the simp, each with its own return: the DOUBTER's word gets agreed with and handed back (a "never work" answered with — for you); the NORMIE COPE gets granted sincerely as a curse (an "enjoy life" answered by wishing them their own commute forever); the STANDARD QUESTION gets its premise removed (a dream-job question answered by never wanting one); the INSULT gets accepted as a compliment with one cold opener; the ADVICE-WORD gets punned into a different life. Standard questions and real slogans count as actually-said — the verbatim law bends only for what people genuinely ask.
...
THE SHAMELESS DOUBLE-DOWN. An indefensible position held with total pride and a syllogism that computes inside its own frame; the reader supplies the correction, and supplying it is the laugh. Escalate, never apologize; land on the baited word; never name the sin. Stock confrontations and cartoon authority beat invented stages. Its interior forms run on the same motor: the DELUSION PRAYER (negotiating with God over which test comes next, faith register played straight while the request is pure greed); VARIANCE PRIDE (the monthly spread that should embarrass, worn as the whole perk); CONSEQUENCE INDIGNATION (sincere disbelief that the crime had rules — one "genuinely" carries it); and the IRONIC SURRENDER (conceding the doubter's whole case deadpan — then proposing the sensible alternative as a 45-year sentence, ideally over footage that refutes the concession).
...
THE BRO LINE. Addressed to one bro, built to be forwarded — the send IS the post working; likes are a side effect. Its forms: the PACT (a we-form vow whose credibility comes from conceding the timeline — months or years, doesn't matter); the RACE (a spendable concrete milestone and a "before you", with a flat lol keeping it love); the DARE (playground-bet grammar carrying seven-figure stakes, period included); the REASSURANCE (an unanswerable rhetorical — why would it be in your head if you couldn't do it — left hanging, no punctuation); the TOUGH LOVE (an apology opener, a hard number of years owed to the game, and a trail-off that closes the door); and PRIDE IN HIM (an edgy setup that swerves wholesome). No punchline required anywhere in this family — recognition of the friendship is the payload, and a joke bolted on breaks the sincerity that makes it sendable.

THE STRAIGHT SERMON. Dead-sincere motivation, zero jokes — and it lives or dies on ONE concrete twist; a plain aphorism anyone could post is the one purpose-failure this family allows. The twists that work: the FUTURE RETROSPECT (put the reader at the end, looking back at the risk that worked); the PROOF REFRAME (rich not for attention — for evidence the life wasn't wasted); the TWO-DISTANCE ledger (concede how far the goal is first, then measure from the start); the STAT REFUTE (quote the one-word doubt, answer with one hard number and nothing else); the PRICED DEFAULT (a rhetorical question that prices the safe life at its true cost, then silence); the VOW (ending on an imagined future scene where someone else speaks the payoff). Sincerity is load-bearing: no irony, no wink, no hedge — and the footage does the flexing so the words never have to.

THE TRIBE NOTE. A quiet interior truth only the ones already chasing it will feel — named plainly, tailed like a footnote (a lowercase btw), and never followed by advice, a push, or a sell. The craft is the understatement: treating a life-defining hunger as a passing observation is what makes it land. Smallest play in the book; run it rarely and it reads like the realest thing on the feed.

THE PORTFOLIO. Some plays are reliable base hits that cap near 8; others can hit 10. A night spends both: bank base hits, take at least one real swing. And plays are FRANCHISES — rerunning a proven play is never the sin; a weak rerun is. Every rerun is judged against the best that play has already produced: bring a sharper pivot or don't bring it back tonight. Some plays rerun well the same night; some burn in one use. Range lives in your catalog — on any given night, ride what's hot. And a real feed never does one JOB all night either: some posts are jokes, some are for the bros, some are dead sincere, some are quiet — spend what tonight's ideas actually deserve.
"""
...
_LAW_CARDS = (
    ...
    # 2026-07-17 re-centering drop — one canon per NEW family/purpose:
    "768d00e0bd0f",  # bro line (the pact — send purpose)
    "c36befaea884",  # straight sermon (future-retrospect — motivational purpose)
    "6d97776dd39b",  # mock-proverb two-layer (funny, proverb costume)
    "d73c57a4c16f",  # video-counterpoint wink (the footage as co-author)
)
